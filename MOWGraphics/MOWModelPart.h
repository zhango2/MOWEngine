...
public:
    ...
    bool                        Serialize(std::ifstream& fIn);
    
    const char*                 MaterialName() const;
    void                        MaterialName(const char* materialName);
    ID3D11Buffer*               GetVertexBuffer();
    ID3D11Buffer*               GetIndexBuffer();
    const char*                 Name()const;
    void                        Name(const char* name);
    void                        ToPb(PbMOWGraphics::PbMOWModelPart& toPb) const;
    static CMOWModelPart*
        FromPb(
...
private:
    ...
    ID3D11Buffer*               m_adjacentIndexBuffer;
    vertex_vector               m_vertices;
    face_vector                 m_faces;
    index_vector                m_indices;
    index_vector                m_indicesWithAdjacencies;
    D3D11_PRIMITIVE_TOPOLOGY    m_topology;
    texture_vector              m_resources;
